...
class CardLayerFactory(ABC):
        ...
        for layer_config in layer_configs:
            ...
            if layer_type == CardLayerType.STATIC_TEXT:
                layers.append(EmbeddedImageTextCardLayer(
                    ...
                    image_provider,
                    # optional
                    max_font_size=layer_config.get('max_font_size') or h.dont_require(
                        config, 'text/max_font_size'),
                    font_file=CardLayerFactory._get_font_file(
                        config, layer_config),
                    spacing_ratio=layer_config.get('spacing_ratio') or h.dont_require(
                        config, 'text/spacing_ratio'),
                    v_alignment=layer_config.get('v_alignment'),
                    h_alignment=layer_config.get('h_alignment'),
                    color=layer_config.get('color') or h.dont_require(
                        config, 'text/color')
                ))
                ...
                layers.append(EmbeddedImageTextCardLayer(
                    ...
                    image_provider,
                    # optional params
                    embedding_map=h.require(
                        config, 'text/embed_symbol_id_map'),
                    max_font_size=layer_config.get('max_font_size') or h.dont_require(
                        config, 'text/max_font_size'),
                    font_file=CardLayerFactory._get_font_file(
                        config, layer_config),
                    ...
                    embed_v_offset_ratio=layer_config.get('embed_v_offset_ratio') or h.dont_require(
                        config, 'text/embed_v_offset_ratio'),
                    embed_size_ratio=layer_config.get('embed_size_ratio') or h.dont_require(
                        config, 'text/embed_size_ratio'),
                    color=layer_config.get('color') or h.dont_require(
                        config, 'text/color')
                ))
    ...
    def _get_font_file(
        ...
        font_file = layer_config.get(
            'font_file') or h.dont_require(config, 'text/font_file')
